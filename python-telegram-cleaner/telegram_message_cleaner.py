...
# Uncomment these lines to provide api_id and api_hash as user input or read from system variables, rather then hardcoding them. The hardcoded api_id and api_has should be commented.
...
# Uncomment these lines to hardcode api_id and api_hash. To provide it as user input or read from system variables, use the code above.
# api_id = 12345
...
async def get_info(chat):
   ...
   # from all users
    messages = (
        await client.get_messages(chat.entity, limit=None) if hasattr(chat, 'entity')
        else await client.get_messages(chat, limit=None)
    )

   # your messages only filtered by "me"
#    #messages = (
